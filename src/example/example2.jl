...
TensorKit.with_blockscheduler(DynamicScheduler()) do
    ...
    D = 2048
    gs, envs, delta = find_groundstate(st, H, DMRG2(trscheme = truncdim(D) & truncerr(1e-6), maxiter=3, verbosity=4));
...
TensorKit.with_blockscheduler(DynamicScheduler()) do
    ...
    filling=filling)
    D = 4096
    gs, envs, delta = find_groundstate(st, H, DMRG2(trscheme = truncdim(D) & truncerr(1e-8), maxiter=5, verbosity=4));
    println(expectation_value(gs, H))
    println(delta)
    save("...jld2", "gs", gs, "delta", delta, "dim", D)
